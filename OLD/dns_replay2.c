/*
  $Id: dns_replay2.c,v 1.20 2017/01/26 08:13:19 fujiwara Exp $
...
BSD:		cc -O -o dns_replay2 dns_replay2.c
 */
...
int send_packet(int s4, int s6, struct addr46 *destp, char *string, int fh)
{
	...
	u_char sendbuf[512], ch;
	u_char *w = sendbuf, *w1, *w2;
	u_char *p, *q;
	struct timeval now;
	u_char c;
	int randlen;
...
** 192.168.1.1 1000 [ 12 34 28 00 / 00 01 00 01 00 01 00 00 / 07 65 78 61 6d 70 6c 65 02 6a 70 00 00 00 00 06 / 00 00 01 00 01 00 00 00 00 00 04 7f 00 00 01 / 00 00 00 00 00 00 00 00 00 00 00 00 00 00 ]
		...
		*w++ = (id & 0xff);
		id++;
		*w++ = flag_rd; /* QR|opcode(4)|AA|TC|RD */
		*w++ = 0; /* RA|Z(3)|OPCODE(4) */
		*w++ = 0; *w++ = 1; /* QDCOUNT */
		*w++ = 0; *w++ = 0; /* ANCOUNT */
		*w++ = 0; *w++ = 0; /* NSCOUNT */
		*w++ = 0; *w++ = edns; /* ANCOUNT */
		...
		while(*p != 0 && *p != '.') {
			...
			while(*p != 0 && *p != '.') {
				...
				if (w - w1 > 63 || w + 9 + 11 > sendbuf+sizeof(sendbuf)) {
					errx(1, "buffer overflow:qname=%s qtype=%d edns=%d wait=%d", qname,qtype,edns, waittime);
				}
				*w++ = ch;
			}
			*w1 = w - w1 - 1;
	...
	header.flag = 'S';
	header.af = destp->u.sa.sa_family;
...
int main(int argc, char *argv[])
{
	...
	char *ofile = NULL;
	int ipv6 = 1;
	long long _Timeout = 15 * 1000 * 1000;
	int opt_l = 0;
	FILE *in = stdin;
	int i;
	int o = 1;
	...
	while((ch = getopt(argc, argv, "6vm:l:i:o:h:p:t:r")) != -1)
	switch(ch) {
	case '6':
		ipv6 = ~ipv6;
		break;
	...
	case 'm':
		mag = atof(optarg);
		if (mag < 0.01 || mag > 100) {
			fprintf(stderr, "-m mag: mag %f overflow\n", mag);
			usage();
		}
		break;
	default:
